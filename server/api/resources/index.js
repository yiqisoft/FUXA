...
module.exports = {
  ...
  app: function () {
          ...
          try {
            ...
            for (var i = 0; i < resourcesDirs.length; i++) {
              ...
              var dirPath = path.resolve(
                runtime.settings.imagesFileDir,
                resourcesDirs[i]
              );
     ...
     */
    resourcesApp.get(
      '/scada/api/resources/generateImage',
      secureFnc,
      function (req, res) {
        var groups = checkGroupsFnc(req);
        if (res.statusCode === 403) {
          runtime.logger.error(
            'api get resources/generateImage: Tocken Expired'
          );
        } else if (authJwt.adminGroups.indexOf(groups) === -1) {
          res
            .status(401)
            .json({ error: 'unauthorized_error', message: 'Unauthorized!' });
          runtime.logger.error(
            'api get resources/generateImage: Unauthorized!'
          );
        } else {
          try {
            ...
            runtime.logger.error(
              'api get resources/generateImage: ' + err.message
            );
          }
        }
      }
...
function getFiles(pathDir, extensions) {
  const filesInDIrectory = fs
    .readdirSync(pathDir)
    .filter(
      (item) => extensions.indexOf(path.extname(item).toLowerCase()) !== -1
    );
