...
module.exports = {
  ...
  app: function () {
     ...
     */
    alarmsApp.get('/scada/api/alarms', secureFnc, function (req, res) {
     ...
     * Take from alarms storage and reply
     */
    alarmsApp.get('/scada/api/alarmsHistory', secureFnc, function (req, res) {
      var groups = checkGroupsFnc(req);
      if (res.statusCode === 403) {
        runtime.logger.error('api get alarms: Tocken Expired');
      } else {
        runtime.alarmsMgr
          .getAlarmsHistory(req.query, groups)
          .then((result) => {
            // res.header("Access-Control-Allow-Origin", "*");
            // res.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept");
            if (result) {
     ...
     * Set alarm ack
     */
    alarmsApp.post('/scada/api/alarmack', secureFnc, function (req, res, next) {
      var groups = checkGroupsFnc(req);
      if (res.statusCode === 403) {
        runtime.logger.error('api post alarms: Tocken Expired');
      } else {
        runtime.alarmsMgr
          .setAlarmAck(req.body.params, req.userId, groups)
          .then(function (data) {
            res.end();
          })
          .catch(function (err) {
