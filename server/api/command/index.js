...
module.exports = {
  ...
  app: function () {
     ...
     * POST build report
     */
    commandApp.post('/scada/api/command', secureFnc, function (req, res, next) {
          ...
          if (req.body.params.cmd === CommanTypeEnum.reportBuild) {
            if (runtime.jobsMgr.forceReport(req.body.params.report)) {
              ...
              runtime.logger.error(
                'api post buildreport: ' + 'report not found!'
              );
            }
          }
        } catch (error) {
     ...
     * GET download
     */
    commandApp.get('/scada/api/download', secureFnc, function (req, res) {
      var groups = checkGroupsFnc(req);
      if (res.statusCode === 403) {
        runtime.logger.error('api post command: Tocken Expired');
      } else if (authJwt.adminGroups.indexOf(groups) === -1) {
        ...
        if (req.query.cmd === CommanTypeEnum.reportDownload) {
          ...
          res
            .status(400)
            .json({ error: 'not_found', message: 'command not found!' });
        }
      }
    });
        ...
        if (res.statusCode === 403) {
          ...
          try {
            ...
            if (tagsIds) {
              ...
              if (errors) {
                runtime.logger.error(
                  'api get getTagValue: ' + 'id not found!' + errors
                );
              }
              res.json(result);
            } else {
              res
                .status(400)
                .json({ error: 'not_found', message: 'tag id not found!' });
          ...
          try {
            if (req.body.tags) {
              ...
              if (errors) {
                res.status(400).json({
                  error: 'not_found',
                  message: 'tag id not found: ' + errors,
                });
                runtime.logger.error(
                  'api post setTagValue: ' + 'id not found!' + errors
                );
              } else {
                res.end();
              }
