...
module.exports = {
  ...
  app: function () {
     ...
     */
    diagnoseApp.get('/scada/api/logsdir', secureFnc, function (req, res) {
     ...
     */
    diagnoseApp.get('/scada/api/logs', secureFnc, function (req, res) {
      var groups = checkGroupsFnc(req);
      if (res.statusCode === 403) {
     ...
     */
    diagnoseApp.get('/scada/api/reportsdir', secureFnc, function (req, res) {
      var groups = checkGroupsFnc(req);
      if (res.statusCode === 403) {
        ...
        try {
          ...
          reportFiles = reportFiles.filter((file) =>
            file.startsWith(req.query.name + '_')
          );
    ...
    diagnoseApp.post(
      ...
      function (req, res, next) {
        ...
        if (res.statusCode === 403) {
          ...
          try {
            if (
              req.body.params.smtp &&
              !req.body.params.smtp.password &&
            ...
            runtime.notificatorMgr
              .sendMail(req.body.params.msg, req.body.params.smtp)
              .then(function () {
                res.end();
              })
              .catch(function (err) {
            ...
            res
              .status(400)
              .json({ error: 'unexpected_error', message: error.toString() });
