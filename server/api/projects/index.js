...
module.exports = {
  ...
  app: function () {
     ...
     */
    prjApp.get('/scada/api/project', secureFnc, function (req, res) {
      var groups = checkGroupsFnc(req);
      runtime.project
        .getProject(req.userId, groups)
        .then((result) => {
     ...
     */
    prjApp.post('/scada/api/project', secureFnc, function (req, res, next) {
      var groups = checkGroupsFnc(req);
     ...
     */
    prjApp.post('/scada/api/projectData', secureFnc, function (req, res, next) {
      var groups = checkGroupsFnc(req);
      if (res.statusCode === 403) {
     ...
     */
    prjApp.get('/scada/api/projectdemo', secureFnc, function (req, res) {
     ...
     * Take from project storage and reply
     */
    prjApp.get('/scada/api/device', secureFnc, function (req, res) {
      var groups = checkGroupsFnc(req);
      if (res.statusCode === 403) {
     ...
     * Set to project storage
     */
    prjApp.post('/scada/api/device', secureFnc, function (req, res, next) {
      var groups = checkGroupsFnc(req);
      if (res.statusCode === 403) {
     ...
     */
    prjApp.post('/scada/api/upload', function (req, res) {
      ...
      try {
        ...
        let result = {
          location:
            '/' + runtime.settings.httpUploadFileStatic + '/' + fileName,
        };
        res.json(result);
      } catch (err) {
        if (err && err.code) {
