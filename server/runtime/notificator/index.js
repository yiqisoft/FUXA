        ...
        .then((alarmsStatus) => {
          Object.keys(alarmsStatus).forEach((stkey) => {
            if (alarmsStatus[stkey]) {
              if (
                notificationsSubsctiption[stkey] &&
                notificationsSubsctiption[stkey].length
              ) {
                var statusChanged =
                  !subscriptionStatus[stkey] ||
                  subscriptionStatus[stkey] < alarmsStatus[stkey];
                for (
                  var i = 0;
                  i < notificationsSubsctiption[stkey].length;
                  i++
                ) {
                  ...
                  if (notification.checkToNotify(time, statusChanged)) {
                    try {
                      ...
                      var alarmsSummary =
                        runtime.alarmsMgr.getAlarmsString(stkey) ||
                        'YiSCADA Alarms Error!';
                      ...
                      runtime.notificatorMgr
                        ...
                        .then(function () {
                          ...
                          logger.info(
                            `notificator.notify.successful: ${new Date()} ${
                              notification.name
                            } ${stkey} ${alarmsSummary}`
                          );
                        })
                        .catch(function (senderr) {
                          logger.error(
                            `notificator.notify.send.failed: ${senderr}`
                          );
                        });
                    } catch (e) {
              ...
              for (
                ...
                notification.reset();
                logger.info(
                  `notificator.notify.toreset: ${notification.name} ${stkey}`
                );
              }
            }
            subscriptionStatus[stkey] = alarmsStatus[stkey];
...
function Notification(id, name, type) {
  ...
  this.checkToNotify = function (time, changed) {
    ...
    if (this.ontime + this.delay * MILLI_MINUTE > time) {
      ...
      this.notifytime &&
      (this.interval <= 0 ||
        this.notifytime + this.interval * MILLI_MINUTE > time)
    ) {
      result = false;
    } else if (changed) {
      // !this.notifytime
