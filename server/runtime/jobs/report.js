...
function Report(_property, _runtime) {
            ...
            .then((filepath) => {
              if (property.receiver) {
                ...
                runtime.notificatorMgr
                  ...
                  .then(function () {
                    logger.info(
                      `report.sended.successful: ${new Date()} ${
                        property.receiver
                      } ${property.name}`
                    );
                  })
  ...
  var _getSampleValues = function (lines, timeRange) {
    ...
    lines.forEach((line) => {
      result[line.id] = [
        { x: timeRange.begin, y: Math.floor(Math.random() * 100) },
  ...
  var _isToExecute = function (date) {
    if (
      inTimeToExecute(date.getHours()) &&
      utils.dayOfYear(lastExecuted) !== utils.dayOfYear(date)
    ) {
      if (property.scheduling === ReportSchedulingType.day) {
        return true;
      } else if (
        property.scheduling === ReportSchedulingType.week &&
        date.getDay() === 1
      ) {
        // monday
        return true;
      } else if (
        property.scheduling === ReportSchedulingType.month &&
        date.getDate() === 1
      ) {
        return true;
      }
  ...
  var _createPdfBinary = function () {
    return new Promise(async function (resolve, reject) {
      ...
      await _getPdfContent(property)
        .then((content) => {
          let docPath = path.join(
            runtime.settings.reportsDir,
            `${property.name}_${utils.getDate(new Date())}.pdf`
          );
    ...
    return new Promise(async function (resolve, reject) {
      try {
        ...
        docDefinition['header'] = {
          text: 'YiSCADA by YIQISOFT',
          style: [{ fontSize: 6 }],
        };
        (docDefinition['footer'] = function (currentPage, pageCount) {
          return {
            text: currentPage.toString() + ' / ' + pageCount,
            style: [{ alignment: 'right', fontSize: 8 }],
          };
        }),
        ...
        for (let i = 0; i < report.content.items.length; i++) {
          ...
          if (item.type === 'text') {
            docDefinition['content'].push({
              text: item.text,
              style: [{ alignment: item.align, fontSize: item.size }],
            });
          } else if (item.type === 'table') {
            await _getTableContent(item).then((itemTable) => {
              const tableDateRange = _getDateRange(item.range);
              docDefinition['content'].push({
                text: `${tableDateRange.begin.toLocaleDateString()} - ${tableDateRange.end.toLocaleDateString()}`,
            ...
            await _getAlarmsContent(item).then((itemAlarms) => {
              ...
              docDefinition['content'].push({
                text: `${alarmsDateRange.begin.toLocaleDateString()} - ${alarmsDateRange.end.toLocaleDateString()}`,
                style: [{ fontSize: item.size }],
              });
    ...
    return new Promise(async function (resolve, reject) {
      try {
        ...
        let header = item.columns.map((col) => {
          return {
            text: col.label || col.tag.label || col.tag.name,
            bold: true,
        ...
        let values = [];
        let tagsids = item.columns
          .filter((col) => col.type !== 0)
          .map((col) => col.tag.id);
        let fncs = item.columns
          .filter((col) => col.type !== 0)
          .map((col) => col.function);
        let formats = item.columns
          .filter((col) => col.type !== 0)
          .map((col) => runtime.devices.getTagFormat(col.tag.id));
  ...
  var _getChartContent = function (itemChart) {
    return new Promise(async function (resolve, reject) {
      try {
        ...
        await _getChartImage(itemChart, values)
          .then((imageData) => {
            content = {
  ...
  var _getDateRange = function (dateRange) {
    if (dateRange === ReportDateRangeType.day) {
      ...
      lastWeek = new Date(
        lastWeek.setDate(lastWeek.getDate() - 7 - ((lastWeek.getDay() + 6) % 7))
      );
      var diff =
        lastWeek.getDate() -
        lastWeek.getDay() +
        (lastWeek.getDay() == 0 ? -6 : 1); // adjust when day is sunday
  ...
  var _getAlarmsContent = function (item) {
    return new Promise(async function (resolve, reject) {
      try {
        ...
        await runtime.alarmsMgr
          .getAlarmsHistory(query)
          .then((result) => {
            if (!result || !result.length) {
              values = [
                Object.values(item.propertyText).map((col) => {
                  return { text: '', style: [{ alignment: 'left' }] };
                }),
              ];
            } else {
              const property = Object.keys(item.property).filter((prop) => {
                if (item.property[prop]) return prop;
              });
              values = result.filter((alr) => {
                if (
                  item.priority[alr.type] &&
                  (!item.alarmFilter ||
                    !!item.alarmFilter.find((name) => name === alr.name))
                ) {
                  return alr;
                }
              });
              values = values.map((alr) => {
                ...
                property.forEach((prop) => {
                  var text = '';
                  if (prop === 'ontime' && alr.ontime)
                    text = utils.getFormatDate(
                      new Date(Number(alr.ontime)),
                      'ymd'
                    );
                  else if (prop === 'offtime' && alr.offtime)
                    text = utils.getFormatDate(
                      new Date(Number(alr.offtime)),
                      'ymd'
                    );
                  else if (prop === 'acktime' && alr.acktime)
                    text = utils.getFormatDate(
                      new Date(Number(alr.acktime)),
                      'ymd'
                    );
                  ...
                  row.push({
                    text: text,
                    style: [{ fillColor: alr.bkcolor, color: alr.color }],
                  });
                });
                return row;
              });
