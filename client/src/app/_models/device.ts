...
export const FuxaServer = { id: "0", name: "YiSCADA" };
...
export class Device {
    ...
    static descriptor = {
        ...
        polling:
            "Polling interval in millisec., check changed value after ask value, by OPCUA there is a monitor",
...
export class Tag {
    ...
    static descriptor = {
        ...
        options:
            "Options is a string JSON object, used for WebAPI and MQTT, pubs: items to publish | subs: items to subscribe",
        init: "Init value",
        daq: {
...
export class DevicesUtils {
    ...
    static getTagFromTagAddress(device: Device, address: string): Tag {
        return <Tag>(
            Object.values(device.tags).find(
                (tag: Tag) => tag.address === address
            )
        );
    }
    ...
    static devicesToCsv(devices: Device[]): string {
        ...
        let devicesData = "";
        const dkeys = Object.keys(Device.descriptor).filter(
            (k) => k !== "tags"
        );
        ...
        let tagsHeader = "";
        let tagsData = "";
        const tkeys = Object.keys(Tag.descriptor).filter(
            (k) => k !== "daq" && k !== "options"
        );
        ...
        for (let i = 0; i < devices.length; i++) {
            if (devices[i].tags) {
                ...
                for (let y = 0; y < tags.length; y++) {
                    tagsData += DevicesUtils.tag2Line(
                        tags[y],
                        devices[i].id,
                        tkeys
                    );
    ...
    static csvToDevices(source: string): Device[] {
        try {
            ...
            const deviceKeyLength =
                Object.keys(Device.descriptor).length +
                Object.keys(DeviceNetProperty.descriptor).length;
            ...
            let devices = {};
            const lines = source
                .split(DevicesUtils.lineDelimiter)
                .filter(
                    (line) =>
                        !line.startsWith(DevicesUtils.lineComment) &&
                        !line.startsWith(DevicesUtils.lineSectionHeader)
                );
            lines.forEach((line) => {
                if (line.startsWith(DevicesUtils.lineDevice)) {
                    // Device
                    let device = DevicesUtils.line2Device(
                        line,
                        deviceKeyLength
                    );
    ...
    static device2Line(
        ...
        dkeys.forEach((dk) => {
            if (dk !== "property") {
                ...
                result += `${text.replace(
                    new RegExp(DevicesUtils.columnDelimiter, "g"),
                    DevicesUtils.columnMaske
                )}${DevicesUtils.columnDelimiter}`;
            }
        });
        if (device.property) {
            pkeys.forEach((pk) => {
                result += `${device.property[pk] || ""}${
                    DevicesUtils.columnDelimiter
                }`;
            });
        }
    ...
    static line2Device(line: string, deviceKeyLength: number): Device {
        ...
        if (items.length < deviceKeyLength - 1) {
            throw new Error(
                `Format Error ${items.length}/${deviceKeyLength} ${line}`
            );
        }
        let device = new Device(items[1]);
        device.name = items[2].replace(
            new RegExp(DevicesUtils.columnMaske, "g"),
            DevicesUtils.columnDelimiter
        );
    ...
    static tag2Line(tag: Tag, deviceId: string, tkeys: string[]): string {
        ...
        result += `${tag.daq ? tag.daq.enabled : ""}${
            DevicesUtils.columnDelimiter
        }`;
        result += `${tag.daq ? tag.daq.interval : ""}${
            DevicesUtils.columnDelimiter
        }`;
        return result;
    }
    ...
    static line2Tag(
        ...
        if (items.length < tagKeyLength - 1) {
            throw new Error(
                `Format Error: ${items.length}/${tagKeyLength} ${line}`
            );
        }
        const deviceId = items[1];
        let tag = new Tag(items[2]);
        tag.name = items[3].replace(
            new RegExp(DevicesUtils.columnMaske, "g"),
            DevicesUtils.columnDelimiter
        );
        tag.label = items[4].replace(
            new RegExp(DevicesUtils.columnMaske, "g"),
            DevicesUtils.columnDelimiter
        );
        tag.type = items[5];
        tag.memaddress = items[6].replace(
            new RegExp(DevicesUtils.columnMaske, "g"),
            DevicesUtils.columnDelimiter
        );
        tag.address = items[7].replace(
            new RegExp(DevicesUtils.columnMaske, "g"),
            DevicesUtils.columnDelimiter
        );
        tag.divisor = parseInt(items[8]) || 1;
        tag.options = items[9].replace(
            new RegExp(DevicesUtils.columnMaske, "g"),
            DevicesUtils.columnDelimiter
        );
