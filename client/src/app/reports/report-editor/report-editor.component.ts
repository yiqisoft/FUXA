...
export class ReportEditorComponent implements OnInit, AfterViewInit {
    ...
    constructor(
        public dialogRef: MatDialogRef<ReportEditorComponent>,
    ...
    ngOnInit() {
        Object.keys(this.schedulingType).forEach((key) => {
            this.translateService
                .get(this.schedulingType[key])
                .subscribe((txt: string) => {
                    this.schedulingType[key] = txt;
                });
        });
    }
    ...
    onSchedulingChanged() {
        this.report.content.items.forEach((item) => {
            if (item.type === this.itemTableType) {
                (<ReportItemTable>item).range =
                    this.myForm.controls.scheduling.value;
            } else if (item.type === this.itemAlarmsType) {
                (<ReportItemAlarms>item).range =
                    this.myForm.controls.scheduling.value;
            }
        });
        this.onReportChanged();
    ...
    onReportChanged() {
        this.report.docproperty.pageMargins = [
            this.myForm.controls.marginLeft.value,
    ...
    getPdfContent(report: Report): Observable<Object> {
        return new Observable((observer) => {
            ...
            this.checkImages(
                report.content.items.filter(
                    (item) => item.type === this.itemChartType
                )
            ).subscribe(
                (images: ImageItem[]) => {
                    ...
                    report.content.items.forEach((item: ReportItem) => {
                        if (item.type === this.itemTextType) {
                            ...
                            const itemTable =
                                ReportEditorComponent.getTableContent(
                                    <ReportItemTable>item
                                );
                            const tableDateRange =
                                ReportEditorComponent.getDateRange(
                                    (<ReportItemTable>item).range
                                );
                            docDefinition["content"].push({
                            ...
                            const itemTable =
                                ReportEditorComponent.getAlarmsContent(
                                    <ReportItemAlarms>item
                                );
                            const tableDateRange =
                                ReportEditorComponent.getDateRange(
                                    (<ReportItemAlarms>item).range
                            ...
                            if (
                                itemChart.chart &&
                                this.imagesList[itemChart.chart.id]
                            ) {
                                docDefinition["content"].push({
    ...
    private checkImages(items: ReportItem[]): Observable<ImageItem[]> {
            ...
            if (chartItem.chart && !this.imagesList[chartItem.chart.id]) {
                source.push(
                    this.resourcesService
                        .generateImage(<ReportItemChart>item)
                        .pipe(
                            map((result) => ({
                                id: (<ReportItemChart>item).chart.id,
                                content: result,
                            }))
                        )
                );
        ...
        return forkJoin(source).pipe(map((results) => [...results]));
    }
    ...
    onAddItem(type: string, index: number = 0, edit: boolean = false) {
        ...
        if (type === this.itemTextType) {
            ...
            item = {
                ...item,
                ...(<ReportItemAlarms>{
                    priority: Utils.convertArrayToObject(
                        Object.values(AlarmsType),
                        true
                    ),
                    property: Utils.convertArrayToObject(
                        Object.values(AlarmPropertyType),
                        true
                    ),
                    range: this.myForm.value.scheduling,
                }),
    ...
    private checkToRemoveImage(index: number) {
        if (
            this.report.content.items[index] &&
            this.report.content.items[index].type === this.itemChartType
        ) {
            const reportChart = <ReportItemChart>(
                this.report.content.items[index]
            );
    ...
    private static getDateRange(dateRange: ReportDateRangeType): DateTimeRange {
        if (
            dateRange ===
            Utils.getEnumKey(ReportDateRangeType, ReportDateRangeType.day)
        ) {
            ...
            dateRange ===
            Utils.getEnumKey(ReportDateRangeType, ReportDateRangeType.week)
        ) {
            var lastWeek = new Date();
            lastWeek = new Date(
                lastWeek.setDate(
                    lastWeek.getDate() - 7 - ((lastWeek.getDay() + 6) % 7)
                )
            );
            var diff =
                lastWeek.getDate() -
            ...
            dateRange ===
            Utils.getEnumKey(ReportDateRangeType, ReportDateRangeType.month)
        ) {
