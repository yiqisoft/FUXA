...
export class AppSettingsComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        for (let i = 0; i < this.authType.length; i++) {
            this.translateService
                .get(this.authType[i].text)
                .subscribe((txt: string) => {
                    this.authType[i].text = txt;
                });
        }
        this.translateService
            .get("dlg.app-auth-tooltip")
            .subscribe((txt: string) => {
    ...
    isSmtpTestReady() {
        ...
        if (
            ...
            !this.settings.smtp.username.length
        ) {
            return false;
        }
