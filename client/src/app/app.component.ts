...
export class AppComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngOnInit() {
        console.log(`YiSCADA v${environment.version}`);
    ...
    ngAfterViewInit() {
        try {
            ...
            this.subscriptionShowLoading =
                this.appService.onShowLoading.subscribe(
                    (show) => {
                        this.isLoading = show;
                    },
                    (error) => {
    ...
    onGoTo(goto) {
        ...
        //TODO!
        if (
            !this.location.path().includes(goto) &&
            ["home", "lab"].indexOf(goto) !== -1
        ) {
            if (goto === "lab") {
